...
if(condActionArithm) {
    ...
    condNumArifm = (firstNumberStr !== null) && (secondNumberStr !== null) && (!isNaN(firstNumber)) && (!isNaN(secondNumber));
    if (condNumArifm) {
        if (kindOfAction === "Add") {
    ...
    firstNumber = Number(firstNumberStr);
    condNumAlg = (firstNumberStr !== null) && (!isNaN(firstNumber));
    if (kindOfAction === "Sqrt") {
        ...
        if (firstNumber >= 0) {
            alert(`Square root of a number ${firstNumber} is ${Math.exp(firstNumber).toFixed(2)}`);
        } else {
