...
(!condNum1 && incorrectAct) ? kindOfAction = 'WNum': a = 'Nothing to do';
...
switch (kindOfAction) {
    ...
    case 'Mult':
        ...
        break
    case  'Div':
        secondNumber === 0 ? alert('Division by 0 is not possible. Good by, see you later.') :
        alert(`Division of ${firstNumber} by ${secondNumber} is ${(firstNumber / secondNumber).toFixed(2)}`);
        break
    case "Sqrt":
        firstNumber <= 0 ? alert('Extracting the square root of a number is not possible. Good by, see you later.') :
        alert(`Square root of a number ${firstNumber} is ${Math.sqrt(firstNumber).toFixed(2)}`);
        break
    case 'Exp':
        firstNumber <= 0 ? alert('Raising to a negative power is impossible. Good by, see you later.') :
    ...
    case 'WNum':
        ...
        break
    default: alert('You entered an INCORRECT ACTION, the program will stop working! Good by, see you later.');
}
