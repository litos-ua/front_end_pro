...
### Д.з. 29 
  Валідація виконувалася для одного окремого імені email за припущенням таких умов:
  - ім'я користувача відділяється від доменного імені знаком '@',
  - в імені користувача можна використовувати латинські літери (як великі, так і
    маленькі), цифри, символи '-' та '.',
  - символи '-' та '.' не можуть бути на першій або останній позиціі імені 
    користувача,
  - усі інші символи неможливо використовувати в імені користувача,
  - після знака '@' іде доменне ім'я,
  - доменне ім'я, як правило, має трехрівневу структуру (з права наліво - домени 
    першого, другого та третього ступенів, відділені між собою '.'),
  - доменне ім'я може також складатися з двох рівнів,
  - правило валідності доменів другого та третього ступенів аналогічні до правил
    валідності ім'я користувача з такими відмінами: дозволяється використання 
    символа '_' і не допускається використання символа '.', а також можливе 
    використання цифри на останній позиції домена цього рівня,
  - домен першого рівня повинен складатися лише з двох або трьох літер латинського
    алфавиту (усі інші символи не можно використовувати).

### Д.з. 30    
   Доопрацьовано версію стартового проєкту на основі створеного на уроці.

1. Додано можливість перезавантаження сторінки браузеру на кожну зміну файлів
   html,css,js,jpg,png,gif. Для цього використано пакет [browserSync]. 
   При кожній зміні файлів зазначеного типу сторінка перезавантажується.
2. Додано можливість стиску графічних файлів за допомогою пакету [gulp-imagemin]
   версії 7.1.0.
3. Додано можливість вибыркового додавання зображень за допомогою пакету
   [gulp-newer] 
4. Додано можливість мініфікації Html коду за допомогою [gulp-minify-html].

### Д.з. 32
  Створено додаток, який складається з таблиці контактів та модальної
форми вводу данних, за допомогою якої до таблиці додаються нові данні.
Дані для базового формування таблиці взяті з API та записані у масив,
який знаходиться у файлі 'src/data/users.js'.
    Для сворення таблиці задіяні лише деякі поля елементів масиву з
ключами 'id', 'name', 'username', 'phone'. Останнім до таблиці
додано стовпчик з кнопками, за допомогою яких можна видалити запис у
таблиці.
    Дані з таблиці зберегаються у LocalStorage з ключем 'tableData'.
В подальшому дані з масиву використовуються лише у випадку коли у
LocalStorage дані з відповідним ключем відсутні.
    Для додавання нових даних використовується модальна форма. Ця форма
відкривається та закривається за допомогою кнопки 'Toggle modal'.У формі
чотире блоки input, які відповідають полям таблиці. Поле 'id' автоінкрементне
та недоступне для корегування. У блоці input тільки виводиться його 
значення. У інші поля вводится інформація для створення нового запису.
Для запису треба натиснути кнопку 'Add user'. Після цього виводиться 
сповіщення про результат запису. При вдалому результаті виводиться
таблиця з новим елементом. За допомогою кнопки 'Cancel' можна скасувати
ввод інформації та закрити форму.
    Для видалення елементу таблиці використовуються кнопки 'Delete',
які знаходяться в окремому полі таблиці. Для видалення треба додатково
підтвердження. Після вдалого видалення елемента виводиться таблиця
вже без нового елементу.

### Д.з. 33 
  Завдання виконано на базі попередньго з використанням функцій та хуків. Додано
декілька проміжних файлів з експортом для уникнення "буріння". Утіліти виніс до
папки 'src/utils'. Константи зі стилями виніс до папки 'src/theme'. 

### Д.з. 34
  Створено локальний TodoList з можливістю його зберігання на віддаленому ресурсі.
Для віддаленого зберігання задач використовано https://mockapi.io/
Реалізовано наступні функції:
 - Додавання todo (за допомогою форми форми) локально та на сервері;
 - Виконання todo по кнопці 'Complete' (закреслення тексту задачі (локально) та зміна 
   значення поля 'completed' на true (локально та на сервері));
 - Видалення todo о кнопці 'Remove' (локально та на сервері));
 - Завантаження у локальне сховище усіх даних з сервера (кнопка 'Load Remote');
 - Очищення локального сховища (кнопка 'Clear Store').
Під час виконання асинхронних операцій працює лоадер.

### Д.з. 35 
  Попереднє завдання перероблено з використанням redux-toolkit,  redux-thunk та бібліотек Material UI,
Formik, Yup, Axios. Збережено попередній функціонал з деяким його розширенням. Це можливість використання
'темної' та 'світлої' тем, валідація форми. Для стилізації компонентів використана бібліотека MUI. Додана
шапка сайту за допомогою AppBar. На ній розміщено перемикач 'темної' та 'світлої' тем. Відправка форми 
виконується за допомогою бібліотеки Formik, валідація за допомогою бібліотеки Yup. Валідація виконується
за наступним принципом:
  - усі поля текстові;
  - поле 'task':
    - мінімальна довжина - 3 символа,
    - максимальна довжина - 20 символів,
    - допустимі символи - букви, цифри,"-", "_",
    - обов'язкове поле.
  - поле 'taskSetter':
    - мінімальна довжина - 5 символа,
    - максимальна довжина - 20 символів,
    - допустимі символи - букви,"-", "_",
    - обов'язкове поле.
  - поле 'taskPerformer':
    - мінімальна довжина - 5 символа,
    - максимальна довжина - 20 символів,
    ...
    - обов'язкове поле.  
  - поле 'additionalInfo':
    - мінімальна довжина - 10 символа,
    - максимальна довжина - 100 символів,
    - необов'язкове поле.   
  Для всіх полів прописані error messages.
